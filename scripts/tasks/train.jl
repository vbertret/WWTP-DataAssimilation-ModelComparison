...
function train_save_model(tasks_parameters)
    ...
    if symbol_model == :nonparametric_black
        ...
        filter_output_train = StateSpaceIdentification.filter(trained_model, y_train_bis, E_train_bis, U_train_bis)
        smoother_output_train = smoother(trained_model, y_train_bis, E_train_bis, U_train_bis, filter_output_train)
    ...
    elseif symbol_model ∈ [:white]
        ...
        p_smoothing = Dict(:n_particles => 5)
        max_iter_em=0
        p_optim_method= Dict(:maxiters =>100)
        t_training = @elapsed begin
