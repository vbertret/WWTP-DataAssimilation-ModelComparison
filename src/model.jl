...
available_model = [:white, :grey, :linear_black, :nonparametric_black, :linear_grey]
...
function get_initial_parameters_system(name::Symbol; kwargs...)
    ...
    @match name begin
        ...
        :linear_black => @match_return vcat([repeat([0.000001], kwargs[:n_Y] * kwargs[:n_Y] + kwargs[:n_Y] + kwargs[:n_Y]*kwargs[:n_E]), repeat([-2.30], 2 * kwargs[:n_Y])]...)
...
function get_bounds_parameters_model(name::Symbol; observed_var=[:nh4], kwargs...)
    ...
    @match name begin
        :white => @match_return vcat([repeat([1e-2], 19), repeat([0], 5+n_Y)]...), vcat([repeat([10e5], 19), repeat([100], 5+n_Y)]...)
...
function get_model(name::Symbol, init_t, x_asm1_init, type_dataset; observed_var=[:nh4], kwargs...)
    ...
    @match name begin
        ...
        :linear_black | :white | :nonparametric_black => begin
            ...
            if name ∈ [:linear_black, :nonparametric_black]
                if type_dataset ∈ [S1Dataset, S2Dataset]
                    x_init = reshape(vcat([x_asm1_init[species, :] for species in observed_var]...), (:))
                elseif type_dataset ∈ [AcigneDataset]
                    x_init = x_asm1_init
                else
            ...
            elseif name == :white
                x_init = [x_asm1_init[2, 1] + x_asm1_init[4, 1], x_asm1_init[8, 1], x_asm1_init[9, 1], x_asm1_init[10, 1], x_asm1_init[11, 1]]
                init_P = x_init.*Matrix(I(5).*0.2)
            end
            init_state = GaussianStateStochasticProcess(init_t, x_init, init_P)

            parameters = get_initial_parameters_system(name, n_Y=n_Y, n_E=n_E; kwargs...)

            if name == :linear_black
                return ForecastingModel{GaussianLinearStateSpaceSystem}(system, init_state, parameters)
            elseif name == :white
                return ForecastingModel{GaussianNonLinearStateSpaceSystem}(system, init_state, parameters)
            elseif name == :nonparametric_black
                return ForecastingModel{GaussianNonParametricStateSpaceSystem}(system, init_state, parameters), update_M
            end
...
function get_nh4_timeseries_from_model(name::Symbol, complete_timeseries, observed_var)
    ...
    @match name begin
        :white => @match_return  TimeSeries{StateSpaceIdentification.ParticleSwarmState}(complete_timeseries.n_t, 1, map(x -> x[4], complete_timeseries))
        :grey => @match_return complete_timeseries
        :linear_black => @match_return TimeSeries{StateSpaceIdentification.GaussianStateStochasticProcess}(complete_timeseries.n_t, 1, map(x -> x[idx_nh4], complete_timeseries))
        :nonparametric_black => @match_return TimeSeries{StateSpaceIdentification.ParticleSwarmState}(complete_timeseries.n_t, 1, map(x -> x[idx_nh4], complete_timeseries))
...
function get_linear_grey_box_system(; dt_model::Int64=1)
    ...
    @inline A_t(exogenous, params, t) = sparse(hcat([[1 - exogenous[1] / params[1] * (dt_model / 1440)]]...))
    @inline B_t(exogenous, params, t) = sparse(hcat([[-params[2] * (dt_model / 1440)]]...))
    @inline c_t(exogenous, params, t) = sparse([(exogenous[2] * exogenous[1]) / params[1] * (dt_model / 1440)])
    ...
    return GaussianLinearStateSpaceSystem(A_t, B_t, c_t, H_t, d_t, R_t, Q_t, n_X, n_Y, dt_model / 1440)
...
function get_non_linear_grey_box_system(; dt_model::Int64=1)
    ...
    @inline M_t(x, exogenous, u, params, t) = sparse([1 - exogenous[1] / params[1] * (dt_model / 1440);;]) * x + sparse([-params[2] * (dt_model / 1440);;]) * u .* (x ./ (x .+ params[3])) .+ sparse([(exogenous[2] * exogenous[1]) / params[1] * (dt_model / 1440)])
    ...
    return GaussianNonLinearStateSpaceSystem(M_t, H_t, R_t, Q_t, n_X, n_Y, dt_model / 1440)
...
function get_white_box_model(; dt_model::Int64=1, observed_var=[:nh4])
    ...
    n_Y = size(observed_var, 1)
    greybox_problem = ODEProblem(ASPSimulator.simplified_asm1!, zeros(6),  (0, 1), repeat([0.0, 25]))
    ...
    @inline Q_t(exogenous, params, t) = PDiagMat(params[25:(25+n_Y-1)] .^ 2)  + ϵ.*I(n_Y)
...
function get_linear_black_box_model(; dt_model::Int64=1, n_Y=1, n_E=2)
    ...
    @inline B_t(exogenous, params, t) = sparse(reshape(params[(n_Y*n_Y+1):(n_Y*n_Y+n_Y)], n_Y, 1))
    @inline c_t(exogenous, params, t) = sparse(reshape(params[(n_Y*n_Y+n_Y+1):(n_Y*n_Y + n_Y + n_Y*n_E)], n_Y, n_E)) * exogenous

    @inline H_t(exogenous, params, t) = sparse(Matrix{Float64}(I, n_Y, n_Y))
    @inline d_t(exogenous, params, t) = sparse(zeros(n_Y))

    @inline R_t(exogenous, params, t) = PDiagMat((params[(n_Y*n_Y + n_Y*n_E + n_Y +1):(n_Y*n_Y + 2*n_Y+ n_Y*n_E)]) .^ 2)
    @inline Q_t(exogenous, params, t) = PDiagMat((params[(n_Y*n_Y+2*n_Y + n_Y*n_E + 1):(n_Y*n_Y+3*n_Y+n_Y*n_E)]) .^ 2)

    return GaussianLinearStateSpaceSystem(A_t, B_t, c_t, H_t, d_t, R_t, Q_t, n_Y, n_Y, dt_model / 1440)
...
function get_nonparametric_black_box_model(; dt_model::Int64=1, n_Y=1, kwargs...)
    ...
    function M_t(x, x_scaled, exogenous, u, llrs, t)
    ...
    function update_M(idx, t_idx, X, E, U, llrs, μ, σ)
        ...
        knn_data = transpose(hcat([StateSpaceIdentification.scale(reshape(X[1:(end-1), :, :], (:, n_X))', μ, σ)', E[idx, :]]...))
        ...
        succesors_data_aeration_on = reshape(succesors_data[repeat(ind_aeration_on[:, 1], n_particules), :], (:, n_X))
        succesors_data_aeration_off = reshape(succesors_data[repeat(.!ind_aeration_on[:, 1], n_particules), :], (:, n_X))
    ...
    @inline Q_t(exogenous, params, t) = PDiagMat(params[(n_Y+1):(2*n_Y)] .^ 2)  + ϵ.*I(n_Y)

    # Define the system
    return GaussianNonParametricStateSpaceSystem(M_t, H_t, R_t, Q_t, n_Y, n_Y, dt_model/(1440), kwargs[:llrs], μ=kwargs[:μ], σ=kwargs[:σ]), update_M
