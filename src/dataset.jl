...
mutable struct S1Dataset <: AbstractDataset
    ...
    function S1Dataset(; seed=1234)
        ...
        init_state, t, x, y, U, E = generate_ASM1_dataset(obs_var, T_steady_state, T_training, T_testing, dt_obs, σ_ϵ, seed; variable_inlet_concentration=false)

        new(name, seed, init_state, T_steady_state, obs_var, dt_obs, σ_ϵ, T_training, T_testing, t, x, y, E, U, 1)
...
mutable struct AcigneDataset <: AbstractDataset
    ...
    function AcigneDataset(; start_date=DateTime("12-04-2023", dateformat"d-m-y"), seed=5, T_training=7.0)
        ...
        # Generate data from the ASM1 Simulation model
        X, Y, E, U, t_index = generate_purecontrol_dataset(start_date, T_training, T_testing, dt_obs, control_id, observation_ids, exogenous_ids)

        init_state = Y[1,:]
        t = getfield.((t_index .- t_index[1]), :value)/(1_000*60*60*24)
        # E = hcat([E, cos.(t*(2*pi)), sin.(t*(2*pi))]...)

        new(name, seed, start_date, init_state, 0.0, obs_var, dt_obs, T_training, T_testing, t, X', Y, E, U, 1)
...
function generate_ASM1_dataset(obs_var, T_steady_state, T_training, T_testing, dt_obs, σ_ϵ, seed; kwargs...)
    ...
    res_steady = ASPSimulator.multi_step!(sim, ASPSimulator.redox_control(system), Day(T_steady_state))
    x_init = sim.state

    # Let the system evolve during T_training + T_testing days to get training and testing data
    sol_asm1  = ASPSimulator.multi_step!(sim, ASPSimulator.redox_control(system), Hour((T_training + T_testing)*24))

    # Postprocess results
    t = vcat([[T_steady_state], sol_asm1.t]...)[1:(end-1)]
    x = hcat([x_init, hcat(sol_asm1.u...)]...)[:, 1:(end-1)]
    Random.seed!(seed);
    y = max.((x[index_obs_var, :] + rand(Normal(0, σ_ϵ), (nb_obs_var, size(x, 2))) + vcat([reshape([(i-1)%Int(dt_obs) == 0 ? 0 : NaN for i in 1:size(x, 2)], (1,size(x, 2))) for i in 1:nb_obs_var]...))', 0)
    U = reshape(x[index_u, :], (:, 1))

    # Get exogenous signals
    Q_in = sim.params.Q_in
    X_in = sim.params.X_in
    Q_in_t = [Q_in(T_steady_state + t/(24*60)) for t in 0:Int((T_training+T_testing)*1440-1)]
    if isa(X_in, Vector{Float64})
        X_in_t = [X_in for t in 0:Int((T_training+T_testing)*1440-1)]
    else
        X_in_t = [X_in(T_steady_state + t/(24*60)) for t in 0:Int((T_training+T_testing)*1440-1)]
    end
...
function adapt_signals_to_model!(d::AbstractDataset, symbol_model::Symbol, dt_model)
    ...
    if typeof(d) ∈ [S1Dataset, S2Dataset]
        ...
        elseif symbol_model ∈ [:white]
            d.E = hcat([d.E[:, 1:1], sum(d.E[:, [3, 5]], dims=2), d.E[:, [9, 10, 11, 12]]]...)
        elseif symbol_model ∈ [:nonparametric_black, :linear_black]
            ASPSimulator.set_default_system(:asm1)
            d.E = d.E[:, vec(vcat([[1], ASPSimulator.get_indexes_from_symbols(d.obs_var) .+ 1]...))]
            if symbol_model == :nonparametric_black
                ...
                d.E[:, 2:end] = StateSpaceIdentification.standard_scaler(d.E[:, 2:end], with_std=false)
            end
        end
    elseif typeof(d) ∈ [AcigneDataset] && symbol_model ∈ [:linear_black, :nonparametric_black]
        if symbol_model == :nonparametric_black
...
function adapt_x_test_model(name::Symbol, x_test, observed_var, type_dataset)
    idx_observed = convert(Array{Int64}, vcat(ASPSimulator.get_indexes_from_symbols.(observed_var)...))
    if type_dataset ∈ [S1Dataset, S2Dataset]
        @match name begin
            :white => @match_return vcat([sum(x_test[[2, 4], :], dims=1), x_test[[8, 9, 10, 11], :]]...)
